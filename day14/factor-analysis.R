...
qplot(1:80, eig_all$values[order(abs(eig_all$values), decreasing=TRUE)][1:80]) # so, 18?
qplot(1:80, eig_male$values[order(abs(eig_male$values), decreasing=TRUE)][1:80]) # maybe 17
qplot(1:80, eig_female$values[order(abs(eig_female$values), decreasing=TRUE)][1:80]) # ..12?
