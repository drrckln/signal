...
for (i in 1:10) {
  ...
  train_sets[[i]] = features[mappings != i, ]
  test_sets[[i]] = features[mappings == i, ]
...
for (alpha in alphas) {
  ...
  for (i in seq(10)) {
    e_fit = glmnet(scale(train_sets[[i]]), extr_sets[[i]], lambda=lambdas, alpha=alpha)
    n_fit = glmnet(scale(train_sets[[i]]), neur_sets[[i]], lambda=lambdas, alpha=alpha)
    e_fits[[i]] = e_fit
    n_fits[[i]] = n_fit
  }
  ...
  for (lambda in lambdas) {
    ...
    for (i in seq(10)) {
      e_predicted[mappings == i] = predict(e_fits[[i]], scale(test_sets[[i]]), s=lambda)
      n_predicted[mappings == i] = predict(n_fits[[i]], scale(test_sets[[i]]), s=lambda)
    }
...
row_selection = as.logical(Map(function(e, n) { e > 0.05107396 | n > 0.048757199 }, coeffs[,1], coeffs[,2]))
