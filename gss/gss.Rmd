...
```{r}
civil_liberties_qs = c("spkrac", "libmslm", "spkath", "colath", "libath", "spkcom", "colcom", "libcom", "spkhomo", "colhomo", "libhomo", "spkmil", "colmil", "libmil", "colrac", "librac", "spkmslm", "colmslm")

# let's do some additional cleaning as well
df$spkrac[df$spkrac == 2] = 0
df$libmslm[df$libmslm == 2] = 0
df$spkath[df$spkath == 2] = 0
df$colath = ifelse(df$colath == 4, 0, ifelse(df$colath == 5, 1, df$colath))
df$libath[df$libath == 2] = 0
df$spkcom[df$spkcom == 2] = 0
df$colcom = ifelse(df$colcom == 5, 0, ifelse(df$colcom == 4, 1, df$colcom))
df$libcom[df$libcom == 2] = 0
df$spkhomo[df$spkhomo == 2] = 0
df$colhomo = ifelse(df$colhomo == 4, 1, ifelse(df$colhomo == 5, 0, df$colhomo))
df$libhomo[df$libhomo == 2] = 0
df$spkmil[df$spkmil == 2] = 0
df$colmil = ifelse(df$colmil == 4, 1, ifelse(df$colmil == 5, 0, df$colmil))
df$libmil[df$libmil == 2] = 0
df$colrac = ifelse(df$colrac == 4, 1, ifelse(df$colrac == 5, 0, df$colrac))
df$librac[df$librac == 2] = 0
df$spkmslm[df$spkmslm == 2] = 0
df$colmslm = ifelse(df$colmslm == 4, 1, ifelse(df$colmslm == 5, 0, df$colmslm))
...
cm = scree(cl_df)
eig_cor_matrix = eigen(cm)
qplot(1:50, eig_cor_matrix$values[order(abs(eig_cor_matrix$values), decreasing=TRUE)][1:50], ylab = "Eigenvalue") # maybe 9?
scree_n = 7 # try 1-7
...
# Grab the related questions for the factors
qs = getQuestions(5, loadings, keys)
...
# could compute the averages of each factor if aggregated by religion
```
